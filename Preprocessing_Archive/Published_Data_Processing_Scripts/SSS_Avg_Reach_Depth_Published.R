# ==============================================================================
#
# Calculate average reach depth (2 hours before data removed during sampling) from 
# depth data 
#
# Status: complete
#
...
for (file in files) {
  ...
  data <- read_csv(file) %>%
    mutate(time_series_average_depth_cm = round(time_series_average_depth_cm, 2))
  ...
  cutoff <- data %>%
    filter(DateTime-lag(data$DateTime)!=15) %>%
    select(DateTime) %>%
    pull()
  
  reach_data <- data %>%
    filter(DateTime < as_datetime(cutoff)) %>%
