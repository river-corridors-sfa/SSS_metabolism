---
title: "Example streamMetabolizer"
author: "Aline Ortega Pieck and Vanessa Garayburu-Caruso"
date: "12/16/2021"
output:
  pdf_document:
    toc: yes
...
knit: (
  function(inputFile, encoding) { 
    ...
    pSubTitle <- paste0("MFC_2021_Linear_pooling_all_data_sigma_pt1_only_june_may_",Sys.Date())
...
setwd(data.path)
 dat = read.csv("SM_WY_2021_only_may_june.csv",header=T)
  file.name = 'MFC_2021_sigma_sigma_pt1_only_june_may_'
  dat = na.omit(dat)
...
labels = c(discharge='discharge\n(m3/S)', temp.water='water temp\n(deg C)', light='PAR\n(umol m^-2 s^-1)')
dat %>% unitted::v() %>%
  select(solar.time, discharge, temp.water, light) %>%
  gather(type, value, discharge, temp.water, light) %>%
  mutate(
    type=ordered(type, levels=c('discharge','temp.water','light')),
...
dat %>% unitted::v() %>%
  select(solar.time, discharge, temp.water, depth) %>%
  gather(type, value, discharge, temp.water, depth) %>%
  mutate(
    type=ordered(type, levels=c('discharge','temp.water','depth')),
    units=ordered(labels[type], unname(labels))) %>%
  ggplot(aes(x=solar.time, y=value, color=type)) + geom_line() +
  facet_grid(units ~ ., scale='free_y') + theme_bw() +
  scale_color_discrete('variable')
...
# Changing the specs
bayes_specs = specs(bayes_name, burnin_steps=1000,
                    saved_steps=500, n_cores=10,
                    K600_daily_sigma_sigma = 0.1)
