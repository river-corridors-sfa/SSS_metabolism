...
#trim weird days
dat <- dat %>%
  filter(!(solar.time >= as.POSIXct("2022-08-07 00:00:00") & solar.time <= as.POSIXct("2022-08-11 00:00:00")))

dat <- dat %>%
  filter(!(solar.time >= as.POSIXct("2022-08-17 00:00:00") & solar.time <= as.POSIXct("2022-08-19 00:00:00")))

dat <- dat %>%
  filter(!(solar.time >= as.POSIXct("2022-08-25 00:00:00") & solar.time <= as.POSIXct("2022-08-27 00:00:00")))
#----------------------------------
...
# Changing the specs
bayes_specs = specs(bayes_name, K600_daily_meanlog_meanlog=log(K600estimate), K600_daily_meanlog_sdlog=0.7, K600_daily_sdlog_sigma=0.01, burnin_steps=1000, 
                  saved_steps=1000)
