...
ERtotal <- read_csv('./Stream_Metabolizer/Outputs/v2_SSS_combined_SM_results.csv', na = '-9999') %>%
  select(Parent_ID, Site_ID, `ERdailymeanmean_gO2/m2day`, `GPPdailymeanmean_gO2/m2day`, ER_removed)
...
all_data <- ERwc_slope %>%
  ...
  rename(Total_Ecosystem_Respiration = "ERdailymeanmean_gO2/m2day",
         ...
         Additional_Days_Removed = ER_removed) %>%
  mutate(Sediment_Respiration = round(Total_Ecosystem_Respiration - Water_Column_Respiration, 2), # caluclate ERsed
...
final_data <- all_data %>%
  select(Parent_ID, Site_ID, Total_Ecosystem_Respiration, Water_Column_Respiration, Sediment_Respiration, Gross_Primary_Production, Additional_Days_Removed)
