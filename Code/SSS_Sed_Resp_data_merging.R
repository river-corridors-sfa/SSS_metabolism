...
data_merge<-function(){
  ...
  # read in ER data 
  sdata<-read.csv('./v2_SSS_Ecosystem_Respiration_Data_Package_STAGING/v2_SSS_Water_Sediment_Total_Respiration_GPP.csv',skip=6)
  
  #model data from kyongho's model
  mdata<-read.csv('./v2_SSS_Ecosystem_Respiration_Data_Package_STAGING/v2_SSS_ER_d50_TotalOxygenConsumed.csv',skip=4)
  sdata<-merge(sdata,mdata,by= c('Parent_ID','Site_ID'))
  sdata[sdata==-9999] = NA
  names(sdata) <-c("Parent_ID","Site_ID","ERtotal_Square","ERwc_Square",       
                   "ERsed_Square",'GPP_Square', "D50_m","Total_Oxygen_Consumed")
  
  # Slope, Discharge, Velocity from national hydrography dataset (NHD)
  nhd_data<-read.csv('./v2_SSS_Ecosystem_Respiration_Data_Package_STAGING/Inputs/v2_SSS_Slope_Discharge_Velocity.csv',skip=6)
  
  # streamcat data 
  scat <-read.csv('./Published_Data/v2_RCSFA_Extracted_Geospatial_Data_2023-06-21.csv')
  ...
  chemdata <- read.csv(file.path('./Published_Data/v3_CM_SSS_Data_Package/Sample_Data','v3_CM_SSS_Water_Sample_Data_Summary.csv'),skip=2)
  chemdata <-chemdata[grep('SSS',chemdata$Sample_Name),]
  chemdata <-chemdata[,c(2,5,6)]; 
  ...
  aflux<-readRDS(file='./Published_Data/nhd_CR_stream_annual_resp_inputs_outputs.rda')[c('COMID','logq_hz_total_m_s')]
  names(aflux)<-c('COMID','hz_annual')
  sflux<-readRDS(file='./Published_Data/nhd_CR_stream_spring_resp_inputs_outputs.rda')[c('COMID','logq_hz_total_m_s')]
  names(sflux)<-c('COMID','hz_spring')
  wflux<-readRDS(file='./Published_Data/nhd_CR_stream_summer_resp_inputs_outputs.rda')[c('COMID','logq_hz_total_m_s')]
  names(wflux)<-c('COMID','hz_winter')
  #fluxs <- Reduce(function(x, y) merge(x, y,by ='COMID',all =TRUE), list(aflux,sflux,wflux))
  gspatial<- read.csv('./Published_Data/v2_RCSFA_Geospatial_Data_Package/v2_RCSFA_Geospatial_Site_Information.csv')[c('COMID','Site_ID')]
  ...
  gdata <- gdata[c("Site_ID","hz_annual", "hz_spring", "hz_winter")]
   ...
   ## merge the data
  cdata <- Reduce(function(x, y) merge(x, y,by ='Parent_ID',all =TRUE), list(sdata,tss,chemdata,depth,chla))
