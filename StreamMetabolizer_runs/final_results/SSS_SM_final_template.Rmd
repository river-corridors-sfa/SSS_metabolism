...
setwd(data.path)
 ...
 K600estimate<-K600estimates[K600estimates$SITE_ID==SITE_ID,3]
...
preds_daily = mm@fit$daily 
#str(preds)
write.csv(preds_daily,paste0(output.path,file.name,"_daily_prediction_results.csv")) 

preds_overall = mm@fit$overall 
#str(preds)
write.csv(preds_overal,paste0(output.path,file.name,"_overall_prediction_results.csv")) 

instant = mm@fit$inst
#str(instant)
write.csv(instant,paste0(output.path,file.name,"_instant_fit_results.csv"))

Overall = mm@fit$overall
#str(Overall)
write.csv(Overall,paste0(output.path,file.name,"_overall_fit_results.csv"))

full = predict_DO(mm)
#str(KQ)
write.csv(full,paste0(output.path,file.name,"_full_prediction_results.csv"))
